...
#         (Replaced old single-index / single-blob logic with multiple indexes
#         and multiple blob storages, as requested. Also replaced old LLM data.)
#######################################################################################
CONFIG = {
    ###############################################################################
    # 1) Azure OpenAI LLM (with placeholders for endpoint + key)
    ###############################################################################
    ...
    ###############################################################################
    # 2) BLOB STORAGE CONFIGS (multiple storages, each with a SAS placeholder)
    #    The directory structure is assumed as:
    #       data/
    #         Tabular/ (for .xlsx, .csv, etc. with table data)
    #         Textual/ (for textual files)
    #         RBAC/
    #            File_rbac.xlsx
    #            User_rbac.xlsx
    #         logs/ (where we store logs)
    ###############################################################################
    "BLOB_STORAGES": [
        {
            "ACCOUNT_URL": "https://enterisesd1.blob.core.windows.net",
            # your SAS token must have sp=rwl (read+write+list) if you want to log
    ...
    ###############################################################################
    # 3) MULTIPLE INDEX CONFIGS
    #    Each item in the list is an index we query. We do top_k=5 from each one.
    ###############################################################################
    "INDEXES": [
        {
...
def _initialize_table_info():
    ...
    all_files_info = {}  # {filename_lower: { 'filename': '...', 'schema': {...}, 'samples': [...]} }

    # Loop over each storage
            ...
            for blob in blobs:
                ...
                if file_ext.endswith(".xlsx") or file_ext.endswith(".xls") or file_ext.endswith(".csv"):
                    ...
                    if key in all_files_info:
                        continue  # skip duplicates if found in multiple storages

                    # We'll attempt to read just to glean schema + sample
                    try:
                        ...
                        # Build the schema (column_name: dtype)
                        schema_dict = {}
                        for col in df.columns:
                            schema_dict[col] = str(df[col].dtype)

                        # Build sample (up to 3 rows)
                        samples = []
                        for i in range(min(3, len(df))):
                            row_data = {}
                            for col in df.columns:
                                # Convert to string with a cutoff
                                val = df.iloc[i][col]
                        ...
                        # If reading fails for some reason, we skip
                        pass
        except Exception:
            # skip this storage if any error
            pass

    # Now build the big textual lumps used in the code
    # 1) TABLES
    table_list_strs = []
...
def agent_answer(user_question, user_tier=1):
    ...
    if is_entirely_greeting_or_punc(user_question_stripped):
        if len(chat_history) < 4:
            yield "Hello! I'm The DGCL AI Assistant. I'm here to help you. What would you like to know today?\n- To reset the conversation type 'restart chat'.\n- To generate Slides, Charts or Document, type 'export followed by your requirements."
        else:
